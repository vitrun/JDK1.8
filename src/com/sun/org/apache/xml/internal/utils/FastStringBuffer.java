...
public class FastStringBuffer {
   ...
   * allocation units, to trade off allocation speed of large document against storage efficiency of
   * small ones.
   * @param maxChunkBits Number of character-offset bits that should be used for addressing within a
   * chunk. Maximum length of a chunk is 2^chunkBits characters.
   ...
   * XMLCharacterRecognizer. <p> CURRENTLY DOES NOT CHECK FOR OUT-OF-RANGE.
   */
   ...
   * recursively to process an inner FSB): <dl> <dt>0</dt> <dd>if this output did not end in
   * retained whitespace, and thus whitespace at the start of the following chunk (if any) should be
   * converted to a single space. <dt>SUPPRESS_LEADING_WS</dt> <dd>if this output ended in retained
   * whitespace, and thus whitespace at the start of the following chunk (if any) should be
   * completely suppressed.</dd> </dd> </dl>
   * @throws org.xml.sax.SAXException may be thrown by handler's characters() method.
   */
   ...
   * contining two flags, bitwise-ORed together: <dl> <dt>SUPPRESS_LEADING_WS</dt> <dd>When false,
   * causes leading whitespace to be converted to a single space; when true, causes it to be
   * discarded entirely. Should be set TRUE for the first chunk, and (in multi-chunk output)
   * whenever the previous chunk ended in retained whitespace.</dd> <dt>SUPPRESS_TRAILING_WS</dt>
   * <dd>When false, causes trailing whitespace to be converted to a single space; when true, causes
   * it to be discarded entirely. Should be set TRUE for the last or only chunk. </dd> </dl>
   * @return normalization status, as in the edgeTreatmentFlags parameter: <dl> <dt>0</dt> <dd>if
   * this output did not end in retained whitespace, and thus whitespace at the start of the
   * following chunk (if any) should be converted to a single space. <dt>SUPPRESS_LEADING_WS</dt>
   * <dd>if this output ended in retained whitespace, and thus whitespace at the start of the
   * following chunk (if any) should be completely suppressed.</dd> </dd> </dl>
