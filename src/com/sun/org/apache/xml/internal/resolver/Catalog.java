 ...
 * Catalog semantics from the SGML days when system identifiers were optional.)</p>
 *
 ...
 *
 * <p>This class recognizes all of the Catalog entries defined in TR9401:1997:</p>
 *
 * <ul> <li><b>BASE</b> changes the base URI for resolving relative system identifiers. The initial
 * base URI is the URI of the location of the catalog (which is, in turn, relative to the location
 * of the current working directory at startup, as returned by the <tt>user.dir</tt> system
 * property).</li> <li><b>CATALOG</b> processes other catalog files. An included catalog occurs
 * logically at the end of the including catalog.</li> <li><b>DELEGATE_PUBLIC</b> specifies
 * alternate catalogs for some public identifiers. The delegated catalogs are not loaded until they
 * are needed, but they are cached once loaded.</li> <li><b>DELEGATE_SYSTEM</b> specifies alternate
 * catalogs for some system identifiers. The delegated catalogs are not loaded until they are
 * needed, but they are cached once loaded.</li> <li><b>DELEGATE_URI</b> specifies alternate
 * catalogs for some URIs. The delegated catalogs are not loaded until they are needed, but they are
 * cached once loaded.</li> <li><b>REWRITE_SYSTEM</b> specifies alternate prefix for a system
 * identifier.</li> <li><b>REWRITE_URI</b> specifies alternate prefix for a URI.</li>
 * <li><b>SYSTEM_SUFFIX</b> maps any system identifier that ends with a particular suffix to another
 * system identifier.</li> <li><b>URI_SUFFIX</b> maps any URI that ends with a particular suffix to
 * another URI.</li> <li><b>DOCTYPE</b> associates the names of root elements with URIs. (In other
 * words, an XML processor might infer the doctype of an XML document that does not include a
 * doctype declaration by looking for the DOCTYPE entry in the catalog which matches the name of the
 * root element of the document.)</li> <li><b>DOCUMENT</b> provides a default document.</li>
 * <li><b>DTDDECL</b> recognized and silently ignored. Not relevant for XML.</li> <li><b>ENTITY</b>
 * associates entity names with URIs.</li> <li><b>LINKTYPE</b> recognized and silently ignored. Not
 * relevant for XML.</li> <li><b>NOTATION</b> associates notation names with URIs.</li>
 * <li><b>OVERRIDE</b> changes the override behavior. Initial behavior is set by the system property
 * <tt>xml.catalog.override</tt>. The default initial behavior is 'YES', that is, entries in the
 * catalog override system identifiers specified in the document.</li> <li><b>PUBLIC</b> maps a
 * public identifier to a system identifier.</li> <li><b>SGMLDECL</b> recognized and silently
 * ignored. Not relevant for XML.</li> <li><b>SYSTEM</b> maps a system identifier to another system
 * identifier.</li> <li><b>URI</b> maps a URI to another URI.</li> </ul>
 *
 ...
 * discarded as a filename would in a URI for a resource: "http://example.com/a/b/c/". </p>
 *
 * <p>Note that subordinate catalogs (all catalogs except the first, including CATALOG and DELEGATE*
 * catalogs) are only loaded if and when they are required.</p>
 *
 ...
 * catalogs, XML catalogs, or any number of other storage formats.</p>
 *
 * <p>Additional catalogs may also be loaded with the {@link #parseCatalog} method.</p> </dd> </dl>
 *
 * <p><b>Change Log:</b></p> <dl> <dt>2.0</dt> <dd><p>Rewrite to use CatalogReaders.</p></dd>
 * <dt>1.1</dt> <dd><p>Allow quoted components in <tt>xml.catalog.files</tt> so that URLs containing
 * colons can be used on Unix. The string passed to <tt>xml.catalog.files</tt> can now have the
 * form:</p>
 * <pre>
 * unquoted-path-with-no-sep-chars:"double-quoted path with or without sep chars":'single-quoted
 * path with or without sep chars'
 * </pre>
 * <p>(Where ":" is the separater character in this example.)</p> <p>If an unquoted path contains an
 * embedded double or single quote character, no special processig is performed on that character.
 * No path can contain separater characters, double, and single quotes simultaneously.</p> <p>Fix
 * bug in calculation of BASE entries: if a catalog contains multiple BASE entries, each is relative
 * to the preceding base, not the default base URI of the catalog.</p> </dd> <dt>1.0.1</dt>
 * <dd><p>Fixed a bug in the calculation of the list of subordinate catalogs. This bug caused an
 * infinite loop where parsing would alternately process two catalogs indefinitely.</p> </dd> </dl>
 *
 * @author Norman Walsh <a href="mailto:Norman.Walsh@Sun.COM">Norman.Walsh@Sun.COM</a>
 * @version 1.0
 *
 *          <p>Derived from public domain code originally published by Arbortext, Inc.</p>
 * @see CatalogReader
 * @see CatalogEntry
 */
public class Catalog {
   ...
   *
   * @throws MalformedURLException One of the system catalogs is identified with a filename that is
   * not a valid URL.
   ...
   * catalogs.</p>
   *
   * @throws MalformedURLException The filename (URL) for a subordinate or delegated catalog is not
   * a valid URL.
   ...
   * if a match is not found in the catalog, instead null is returned to indicate that no match was
   * found.
   * @throws MalformedURLException The formal system identifier of a subordinate catalog cannot be
   * turned into a valid URL.
   * @throws IOException Error reading subordinate catalog file.
