...
public class DTMManagerDefault extends DTMManager {
   ...
   * offset to the node-number field of the public DTM Handle. For the first DTM ID accessing each
   * DTM, this is 0; for overflow addressing it will be a multiple of 1<<IDENT_DTM_NODE_BITS.
   */
  synchronized public void addDTM(DTM dtm, int id, int offset) {
    if (id >= IDENT_MAX_DTMS) {
      ...
      throw new DTMException(XMLMessages.createXMLMessage(XMLErrorResources.ER_NO_DTMIDS_AVAIL,
  ...
  synchronized public int getDTMHandleFromNode(org.w3c.dom.Node node) {
    if (null == node) {
      throw new IllegalArgumentException(XMLMessages
          .createXMLMessage(XMLErrorResources.ER_NODE_NON_NULL,
    ...
    if (node instanceof com.sun.org.apache.xml.internal.dtm.ref.DTMNodeProxy) {
      ...
      Node root = node;
      Node p =
          (root.getNodeType() == Node.ATTRIBUTE_NODE) ? ((org.w3c.dom.Attr) root).getOwnerElement()
              : root.getParentNode();
      ...
      if (DTM.NULL == handle) {
        throw new RuntimeException(XMLMessages
            .createXMLMessage(XMLErrorResources.ER_COULD_NOT_RESOLVE_NODE,
   ...
   * not actually release the DTM. This is intended to support intelligent caching of documents...
   * which is not implemented in this version of the DTM manager.
