...
final class IncrementalSAXSource_Filter
    implements IncrementalSAXSource, ContentHandler, DTDHandler, LexicalHandler, ErrorHandler,
    Runnable {
  ...
  public void init(CoroutineManager co, int controllerCoroutineID,
    ...
    if (fControllerCoroutineID == -1 || fSourceCoroutineID == -1) {
      throw new RuntimeException(XMLMessages
          .createXMLMessage(XMLErrorResources.ER_COJOINROUTINESET_FAILED,
   ...
   * do...() methods, applications should only need to talk to the CoroutineManager once, to obtain
  ...
  public void startParse(InputSource source) throws SAXException {
    if (fNoMoreEvents) {
      throw new SAXException(XMLMessages
          .createXMLMessage(XMLErrorResources.ER_INCRSAXSRCFILTER_NOT_RESTARTABLE,
              null)); //"IncrmentalSAXSource_Filter not currently restartable.");
    }
    if (fXMLReader == null) {
      throw new SAXException(XMLMessages
          .createXMLMessage(XMLErrorResources.ER_XMLRDR_NOT_BEFORE_STARTPARSE,
  ...
  public void run() {
    ...
    try {
      ...
      if (inner instanceof StopException) {
        ...
        if (DEBUG) {
          System.out
              .println("Active IncrementalSAXSource_Filter UNEXPECTED SAX exception: " + inner);
   ...
   *
   * @param parsemore If true, tells the incremental filter to generate another chunk of output. If
   * false, tells the filter that we're satisfied and it can terminate parsing of this document.
   ...
   * deliverMoreNodes(false). Or an exception object if something malfunctioned. %REVIEW% We _could_
   * actually throw the exception, but that would require runinng deliverMoreNodes() in a
   * try/catch... and for many applications, exception will be simply be treated as "not TRUE" in
   * any case.
   */
