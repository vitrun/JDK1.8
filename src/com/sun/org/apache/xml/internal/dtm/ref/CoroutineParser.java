...
public interface CoroutineParser {
   ...
   * unless you know which CoroutineManager you're talking to. Also note that the do...() methods
   * encapsulate the common transactions with the CoroutineParser, so you shouldn't need this in
   * most cases.
   */
   ...
   * methods, applications should only need to talk to the CoroutineManager once, to obtain the
   ...
   * @param appCoroutine The coroutine ID number of the coroutine invoking this method, so it can be
   * resumed after the parser has responded to the request.
   * @return Boolean.TRUE if the CoroutineParser believes more data may be available for further
   * parsing. Boolean.FALSE if parsing ran to completion. Exception if the parser objected for some
   * reason.
   */
   ...
   * CoroutineParser's coroutine ID number.
   *
   * @param parsemore If true, tells the incremental parser to generate another chunk of output. If
   * false, tells the parser that we're satisfied and it can terminate parsing of this document.
   * @param appCoroutine The coroutine ID number of the coroutine invoking this method, so it can be
   * resumed after the parser has responded to the request.
   * @return Boolean.TRUE if the CoroutineParser believes more data may be available for further
   * parsing. Boolean.FALSE if parsing ran to completion. Exception if the parser objected for some
