...
public class DTMDocumentImpl
   ...
   * high bits and returned as part of all node ID numbers, so those IDs indicate which document
   * they came from as well as a location within the document. It is the DTMManager's responsibility
   * to assign a unique number to each document.
   */
   ...
   * built-in SAX ContentHandler, the IncrementalSAXSource if we're bound to one and should receive
...
//      int charStringStart=0,charStringLength=0;
   ...
   * @return The attribute node handle with the specified name ( <code>nodeName</code>) or
   * <code>DTM.NULL</code> if there is no such attribute.
   */
   ...
   * @param prefixIndex: Index within the prefix string pool
   * @param namespaceIndex: Index within the namespaceURI string pool
   * @param isID: If someone really insists on writing a bad DTD, it is theoretically possible for a
   * namespace declaration to also be declared as being a node ID. I don't really want to support
