...
public class CoroutineManager {
   ...
   * ID. Ponder.</p>
   *
   * @param coroutineID If >=0, requests that we reserve this number. If <0, requests that we find,
   * reserve, and return an available ID number.
   * @return If >=0, the ID number to be used by this coroutine. If <0, an error occurred -- the ID
   * requested was already in use, or we couldn't assign one without going over the "unreasonable
   * value" mark
   */
   ...
   * resumed.
   * @throws java.lang.NoSuchMethodException if thisCoroutine isn't a registered member of this
   ...
   * @param thisCoroutine Integer identifier for this coroutine. This is the ID we watch for to see
   * if we're the ones being resumed.
   * @param toCoroutine Integer identifier for the coroutine we wish to invoke.
   * @throws java.lang.NoSuchMethodException if toCoroutine isn't a registered member of this group.
  ...
  public synchronized Object co_resume(Object arg_object, int thisCoroutine, int toCoroutine)
    ...
    if (!m_activeIDs.get(toCoroutine)) {
      throw new java.lang.NoSuchMethodException(XMLMessages
          .createXMLMessage(XMLErrorResources.ER_COROUTINE_NOT_AVAIL, new Object[]{
              Integer.toString(toCoroutine)})); //"Coroutine not available, id="+toCoroutine);
    }
