...
public class XPathParser {
   ...
   * terminates at either the beginning of the string or on a '|' character.  Because of this, this
   * method should only be used for pattern matching.
  ...
  protected void BooleanExpr() throws javax.xml.transform.TransformerException {
    ...
    if (opLen == 2) {
      error(XPATHErrorResources.ER_BOOLEAN_ARG_NO_LONGER_OPTIONAL,
          null);  //"boolean(...) argument is no longer optional with 19990709 XPath draft.");
    }
   ...
   * more Predicates; FILTER_MATCH_PRIMARY, if this method successfully matched a FilterExpr that
   * was just a PrimaryExpr; or FILTER_MATCH_FAILED, if this method did not match a FilterExpr
   * @throws XSLProcessorException thrown if the active ProblemListener and XPathContext decide the
   * error condition is severe enough to halt processing.
   */
  ...
  protected boolean PrimaryExpr() throws javax.xml.transform.TransformerException {
    ...
    if ((m_tokenChar == '\'') || (m_tokenChar == '"')) {
      ...
      matchFound = true;
        ...
        (('.' == m_tokenChar) && (m_token.length() > 1) && Character.isDigit(
    ...
    if (tokenIs(".")) {
      ...
      if (tokenIs('[')) {
        error(XPATHErrorResources.ER_PREDICATE_ILLEGAL_SYNTAX,
            null);  //"'..[predicate]' or '.[predicate]' is illegal syntax.  Use 'self::node()[predicate]' instead.");
      }
