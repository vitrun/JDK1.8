...
public class ExsltSets extends ExsltBase {
   ...
   * @return a NodeList containing the nodes in nl1 that precede in document order the first node in
   * nl2; an empty node-set if the first node in nl2 is not in nl1; all of nl1 if nl2 is empty.
   * @see <a href="http://www.exslt.org/">EXSLT</a>
   */
   ...
   * @param nl1 NodeList for first node-set.
   * @param nl2 NodeList for second node-set.
   * @return a NodeList containing the nodes in nl1 that follow in document order the first node in
   * nl2; an empty node-set if the first node in nl2 is not in nl1; all of nl1 if nl2 is empty.
   * @see <a href="http://www.exslt.org/">EXSLT</a>
   */
   ...
   * any nodes with the node set passed as the second argument. If there are no nodes that are in
   * both node sets, then it returns false.
   *
   * The Xalan extensions MethodResolver converts 'has-same-node' to 'hasSameNode'.
   *
   * Note: Not to be confused with hasSameNodes in the Xalan namespace, which returns true if the
   * two node sets contain the exactly the same nodes (perhaps in a different order), otherwise
   * false.
   *
   * @see <a href="http://www.exslt.org/">EXSLT</a>
   */
