...
public interface LocalClientRequestDispatcher {
   ...
   * @param operation a string containing the operation name. The operation name corresponds to the
   * operation name as it would be encoded in a GIOP request.
   * @param expectedType a Class object representing the expected type of the servant. The expected
   * type is the Class object associated with the operations class of the stub's interface (e.g. A
   * stub for an interface Foo, would pass the Class object for the FooOperations interface).
   * @return a ServantObject object. The method may return a null value if it does not wish to
   * support this optimization (e.g. due to security, transactions, etc). The method must return
   * null if the servant is not of the expected type.
   */
