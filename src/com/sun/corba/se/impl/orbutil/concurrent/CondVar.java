 ...
 * </pre>
 *
 * @see Mutex <p>[<a href="http://gee.cs.oswego.edu/dl/classes/EDU/oswego/cs/dl/util/concurrent/intro.html">
...
public class CondVar {
   ...
   * exclusion lock.
   *
   * @param mutex A mutual exclusion lock which must either be non-reentrant, or else be
   * ReentrantMutex. Standard usage is to supply an instance of <code>Mutex</code>, but, for
   * example, a Semaphore initialized to 1 also works. On the other hand, many other Sync
   * implementations would not work here, so some care is required to supply a sensible
   * synchronization object. In normal use, the mutex should be one that is used for <em>all</em>
   * synchronization of the object using the CondVar. Generally, to prevent nested monitor lockouts,
   * this object should not use any native Java synchronized blocks.
   **/
   ...
   * even if interrupted.
   *
   * @throws InterruptedException if the thread was interrupted before or during the wait. However,
   * if the thread is interrupted after the wait but during mutex re-acquisition, the interruption
   * is ignored, while still ensuring that the currentThread's interruption state stays true, so can
   ...
   * even if interrupted.
   *
   * @param msecs The time to wait. A value less than or equal to zero causes a momentarily release
   * and re-acquire of the mutex, and always returns false.
   * @return false if at least msecs have elapsed upon resumption; else true. A false return does
   * NOT necessarily imply that the thread was not notified. For example, it might have been
   * notified after the time elapsed but just before resuming.
