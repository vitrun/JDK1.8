...
/**
 * Main interface for locks, gates, and conditions. <p> Sync objects isolate waiting and
 * notification for particular logical states, resource availability, events, and the like that are
 * shared across multiple threads. Use of Syncs sometimes (but by no means always) adds flexibility
 * and efficiency compared to the use of plain java monitor methods and locking, and are sometimes
 * (but by no means always) simpler to program with. <p>
 *
 * Most Syncs are intended to be used primarily (although not exclusively) in  before/after
 ...
 * }
 * </pre>
 * Syncs may be used in somewhat tedious but more flexible replacements for built-in Java
 ...
 * }
 * </pre>
 * If you have a lot of such methods, and they take a common form, you can standardize this using
 * wrappers. Some of these wrappers are standardized in LockedExecutor, but you can make others. For
 * example:
 * <pre>
 * class HandSynchedV2 {
 *   private double state_ = 0.0;
 ...
 * }
 * </pre>
 * <p> One reason to bother with such constructions is to use deadlock- avoiding back-offs when
 * dealing with locks involving multiple objects. For example, here is a Cell class that uses
 * attempt to back-off and retry if two Cells are trying to swap values with each other at the same
 * time.
 * <pre>
 * class Cell {
 *   long value;
 ...
 * }
 * </pre>
 * <p> Here is an even fancier version, that uses lock re-ordering upon conflict:
 * <pre>
 * class Cell {
 *   long value;
 *   Sync lock = ...;
 ...
 * }
 * </pre>
 * <p> Interruptions are in general handled as early as possible. Normally, InterruptionExceptions
 * are thrown in acquire and attempt(msec) if interruption is detected upon entry to the method, as
 * well as in any later context surrounding waits. However, interruption status is ignored in
 * release(); <p> Timed versions of attempt report failure via return value. If so desired, you can
 * transform such constructions to use exception throws via
 * <pre>
 *   if (!c.attempt(timeval)) throw new TimeoutException(timeval);
 * </pre>
 * <p> The TimoutSync wrapper class can be used to automate such usages. <p> All time values are
 * expressed in milliseconds as longs, which have a maximum value of Long.MAX_VALUE, or almost
 * 300,000 centuries. It is not known whether JVMs actually deal correctly with such extreme values.
 * For convenience, some useful time values are defined as static constants. <p> All implementations
 * of the three Sync methods guarantee to somehow employ Java <code>synchronized</code> methods or
 * blocks, and so entail the memory operations described in JLS chapter 17 which ensure that
 * variables are loaded and flushed within before/after constructions. <p> Syncs may also be used in
 * spinlock constructions. Although it is normally best to just use acquire(), various forms of busy
 * waits can be implemented. For a simple example (but one that would probably never be preferable
 * to using acquire()):
 * <pre>
 * class X {
 *   Sync lock = ...
 ...
 * }
 * </pre>
 * <p> In addition pure synchronization control, Syncs may be useful in any context requiring
 * before/after methods. For example, you can use an ObservableSync (perhaps as part of a
 * LayeredSync) in order to obtain callbacks before and after each method invocation for a given
 * class. <p>
 *
...
public interface Sync {
   ...
   * not to wait at all. However, this may still require access to a synchronization lock, which can
   * impose unbounded delay if there is a lot of contention among threads.
