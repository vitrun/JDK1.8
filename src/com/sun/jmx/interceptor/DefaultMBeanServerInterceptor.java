...
/**
 * This is the default class for MBean manipulation on the agent side. It contains the methods
 * necessary for the creation, registration, and deletion of MBeans as well as the access methods
 * for registered MBeans. This is the core component of the JMX infrastructure. <P> Every MBean
 * which is added to the MBean server becomes manageable: its attributes and operations become
 * remotely accessible through the connectors/adaptors connected to that MBean server. A Java object
 * cannot be registered in the MBean server unless it is a JMX compliant MBean. <P> When an MBean is
 * registered or unregistered in the MBean server an {@link javax.management.MBeanServerNotification
 * MBeanServerNotification} Notification is emitted. To register an object as listener to
 * MBeanServerNotifications you should call the MBean server method {@link #addNotificationListener
 * addNotificationListener} with <CODE>ObjectName</CODE> the <CODE>ObjectName</CODE> of the {@link
 * javax.management.MBeanServerDelegate MBeanServerDelegate}. This <CODE>ObjectName</CODE> is: <BR>
...
public class DefaultMBeanServerInterceptor implements MBeanServerInterceptor {
   ...
   * @throws InstanceAlreadyExistsException if an object is already registered with that name.
   * @throws MBeanRegistrationException if an exception occurs during registration.
   **/
   ...
   *
   * @return a ResourceContext for special resources such as ClassLoaders or JMXNamespaces.
   */
   ...
   * repository.
   *
   * @param loader The ClassLoader MBean being registered or unregistered.
   * @param logicalName The name of the ClassLoader MBean.
   * @return a ResourceContext that takes in charge the addition or removal of the loader to or from
   * the CLR.
   */
