...
public class ThreadContext implements Cloneable {
   ...
   * @return the last Object that was pushed (using <code>push</code>) with that key and not
   * subsequently canceled by a <code>restore</code>; or null if there is no such object. A null
   * return value may also indicate that the last Object pushed was the value <code>null</code>.
   * Use the <code>contains</code> method to distinguish this case from the case where there is no
   * Object.
...
//  }
   ...
   * here.</p>
   *
   * @param key the key that will be used to find the object while it is on the stack.
   * @param value the value to be associated with that key.  It may be null.
   * @return a ThreadContext that can be given to <code>restore</code> to restore the stack to its
   * state before the <code>push</code>.
   * @throws IllegalArgumentException if <code>key</code> is null.
   */
   ...
   *
   * @param oldContext the state to return.  This is usually the return value of an earlier
   * <code>push</code> operation.
   * @throws NullPointerException if <code>oldContext</code> is null.
   * @throws IllegalArgumentException if <code>oldContext</code> does not represent a context from
   * this thread, or if that context was undone by an earlier <code>restore</code>.
   */
   ...
   *
   * @throws IllegalArgumentException if the context stack was not empty before the call.  An
   * implementation may also throw this exception if <code>context</code> is no longer current in
   * the thread from which it was obtained.
   */
