 ...
 * proxy.</p>
 *
 * @see <a href="../MXBean.html#custom">MXBean specification, section "Custom MXBean type
 * mappings"</a>
 */
public abstract class MXBeanMappingFactory {
   ...
   * @param f the original mapping factory that was consulted to do the mapping.  A mapping factory
   * should pass this parameter intact if it forwards a type to another mapping factory.  In the
   * example, this is how {@code MyLinkedListMappingFactory} works for types like {@code
   * MyLinkedList[]} and {@code List<MyLinkedList>}.
   * @return the mapping for the given type.
   * @throws OpenDataException if this type cannot be mapped.  This exception is appropriate if the
   * factory is supposed to handle all types of this sort (for example, all linked lists), but
