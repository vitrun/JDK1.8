...
public class RoleList extends ArrayList<Object> {
   ...
   * specified.
   *
   * @param list the {@code List} that defines the initial contents of the new {@code RoleList}.
   * @throws IllegalArgumentException if the {@code list} parameter is {@code null} or if the {@code
   * list} parameter contains any non-Role objects.
   ...
   *
   * @return a {@code List<Role>} whose contents reflect the contents of this {@code RoleList}.
   *
   * <p>If this method has ever been called on a given {@code RoleList} instance, a subsequent
   * attempt to add an object to that instance which is not a {@code Role} will fail with an {@code
   * IllegalArgumentException}. For compatibility reasons, a {@code RoleList} on which this method
   * has never been called does allow objects other than {@code Role}s to be added.</p>
   * @throws IllegalArgumentException if this {@code RoleList} contains an element that is not a
   * {@code Role}.
   * @since 1.6
   */
   ...
   * shifted up.
   *
   * @param index The position in the list where the new Role object is to be inserted.
   ...
   *
   * @param index Position at which to insert the first element from the RoleList specified.
