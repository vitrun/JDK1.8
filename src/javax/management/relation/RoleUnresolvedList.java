...
public class RoleUnresolvedList extends ArrayList<Object> {
   ...
   * list} parameter contains any non-RoleUnresolved objects.
   ...
   *
   * @return a {@code List<RoleUnresolved>} whose contents reflect the contents of this {@code
   * RoleUnresolvedList}.
   *
   * <p>If this method has ever been called on a given {@code RoleUnresolvedList} instance, a
   * subsequent attempt to add an object to that instance which is not a {@code RoleUnresolved} will
   * fail with an {@code IllegalArgumentException}. For compatibility reasons, a {@code
   * RoleUnresolvedList} on which this method has never been called does allow objects other than
   * {@code RoleUnresolved}s to be added.</p>
   * @throws IllegalArgumentException if this {@code RoleUnresolvedList} contains an element that is
   * not a {@code RoleUnresolved}.
   * @since 1.6
   */
   ...
   * shifted up.
   *
   * @param index - The position in the list where the new RoleUnresolved object is to be inserted.
   * @param role - The RoleUnresolved object to be inserted.
   * @throws IllegalArgumentException if the unresolved role is null.
   * @throws IndexOutOfBoundsException if index is out of range (<code>index &lt; 0 || index &gt;
   * size()</code>).
   */
   ...
   * @param role - The value to which the unresolved role element should be set.
   * @throws IllegalArgumentException if the unresolved role is null.
   * @throws IndexOutOfBoundsException if index is out of range (<code>index &lt; 0 || index &gt;=
   * size()</code>).
   */
   ...
   * @throws IndexOutOfBoundsException if accessing with an index outside of the list.
   */
   ...
   *
   * @param index - Position at which to insert the first element from the RoleUnresolvedList
   * specified.
