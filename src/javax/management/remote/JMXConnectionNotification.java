...
public class JMXConnectionNotification extends Notification {
   ...
   * </ul>
   *
   * @param type the type of the notification.  This is usually one of the constants {@link
   * #OPENED}, {@link #CLOSED}, {@link #FAILED}, {@link #NOTIFS_LOST}.  It is not an error for it to
   * be a different string.
   * @param source the connector server or client emitting the notification.
   * @param connectionId the ID of the connection within its connector server.
   * @param sequenceNumber a non-negative integer.  It is expected but not required that this number
   * will be greater than any previous <code>sequenceNumber</code> in a notification from this
   * source.
   * @param message an unspecified text message, typically containing a human-readable description
   * of the event.  Can be null.
   * @param userData an object whose type and meaning is defined by the connector server.  Can be
   * null.
   * @throws NullPointerException if <code>type</code>, <code>source</code>, or
   * <code>connectionId</code> is null.
   * @throws IllegalArgumentException if <code>sequenceNumber</code> is negative.
   */
