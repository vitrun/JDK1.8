...
public class RMIConnector implements JMXConnector, Serializable, JMXAddressable {
   ...
   * @param environment additional attributes specifying how to make the connection.  For JNDI-based
   * addresses, these attributes can usefully include JNDI attributes recognized by {@link
   * InitialContext#InitialContext(Hashtable) InitialContext}.  This parameter can be null, which is
   ...
   * @param environment additional attributes specifying how to make the connection.  This parameter
   * can be null, which is equivalent to an empty Map.
   * @throws IllegalArgumentException if <code>rmiServer</code> is null.
   */
  ...
  /**
   * @throws IOException if the connection could not be made because of a communication problem, or
   * in the case of the {@code iiop} protocol, that RMI/IIOP is not supported
   */
   ...
   * @throws IllegalArgumentException if the <tt>java.naming.corba.orb</tt> property is specified
   * and does not point to an {@link org.omg.CORBA.ORB ORB}.
   ...
   * @throws InvalidObjectException if none of <var>rmiServer</var> stub or <var>jmxServiceURL</var>
   * are set.
   ...
   *
   * @param jndiURL A JNDI URL indicating the location of the Stub (see {@link
   * javax.management.remote.rmi}), e.g.: <ul><li><tt>rmi://registry-host:port/rmi-stub-name</tt></li>
   * <li>or <tt>iiop://cosnaming-host:port/iiop-stub-name</tt></li> <li>or
   * <tt>ldap://ldap-host:port/java-container-dn</tt></li> </ul>
   ...
   *
   * @throws IllegalArgumentException if c is not in the Base64 Alphabet.
   */
  ...
  //--------------------------------------------------------------------
  /**
   * @serial The RMIServer stub of the RMI JMX Connector server to which this client connector is
   * (or will be) connected. This field can be null when <var>jmxServiceURL</var> is not null. This
   * includes the case where <var>jmxServiceURL</var> contains a serialized RMIServer stub. If both
   * <var>rmiServer</var> and <var>jmxServiceURL</var> are null then serialization will fail.
   * @see #RMIConnector(RMIServer, Map)
   **/
  private final RMIServer rmiServer;

  /**
   * @serial The JMXServiceURL of the RMI JMX Connector server to which this client connector will
   * be connected. This field can be null when <var>rmiServer</var> is not null. If both
   * <var>rmiServer</var> and <var>jmxServiceURL</var> are null then serialization will fail.
   * @see #RMIConnector(JMXServiceURL, Map)
   **/
