...
public interface Descriptor extends Serializable, Cloneable {
   ...
   * value for the field.
   * @throws RuntimeOperationsException if the field name or field value is illegal (wrapped
   * exception is {@link IllegalArgumentException}); or if the descriptor is immutable (wrapped
   * exception is {@link UnsupportedOperationException}).
   */
   ...
   *
   * @return String array of fields in the format <i>fieldName=fieldValue</i> <br>If the value of a
   * field is not a String, then the toString() method will be called on it and the returned value,
   * enclosed in parentheses, used as the value for the field in the returned array. If the value of
   * a field is null, then the value of the field in the returned array will be empty.  If the
   * descriptor is empty, you will get an empty array.
   * @see #setFields
   */
   ...
   *
   * @return String array of field names. If the descriptor is empty, you will get an empty array.
   */
   ...
   *
   * @param fieldNames String array of the names of the fields that the values should be returned
   * for.  If the array is empty then an empty array will be returned.  If the array is null then
   * all values will be returned, as if the parameter were the array returned by {@link
   * #getFieldNames()}.  If a field name in the array does not exist, including the case where it is
   * null or the empty string, then null is returned for the matching array element being returned.
   * @return Object array of field values. If the list of {@code fieldNames} is empty, you will get
   * an empty array.
   */
   ...
   * {@link IllegalArgumentException} if {@code fieldNames} or {@code fieldValues} is null, or if
   * the arrays are of different lengths, or if there is an illegal value in one of them. Wrapped
   * exception is {@link UnsupportedOperationException} if the descriptor is immutable, and the call
   ...
   * @throws RuntimeOperationsException If the validity checking fails for any reason, this
   * exception will be thrown. The method returns false if the descriptor is not valid, but throws
