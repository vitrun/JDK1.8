...
public class MBeanInfo implements Cloneable, Serializable, DescriptorRead {
  ...
  /**
   * @serial The Descriptor for the MBean.  This field can be null, which is equivalent to an empty
   * Descriptor.
   */
   ...
   *
   * @param className The name of the Java class of the MBean described by this
   * <CODE>MBeanInfo</CODE>.  This value may be any syntactically legal Java class name.  It does
   * not have to be a Java class known to the MBean server or to the MBean's ClassLoader.  If it is
   * a Java class known to the MBean's ClassLoader, it is recommended but not required that the
   * class's public methods include those that would appear in a Standard MBean implementing the
   * attributes and operations in this MBeanInfo.
   * @param description A human readable description of the MBean (optional).
   * @param attributes The list of exposed attributes of the MBean. This may be null with the same
   * effect as a zero-length array.
   * @param constructors The list of public constructors of the MBean.  This may be null with the
   * same effect as a zero-length array.
   * @param operations The list of operations of the MBean.  This may be null with the same effect
   * as a zero-length array.
   * @param notifications The list of notifications emitted.  This may be null with the same effect
   * as a zero-length array.
   */
   ...
   * @param notifications The list of notifications emitted.  This may be null with the same effect
   * as a zero-length array.
   * @param descriptor The descriptor for the MBean.  This may be null which is equivalent to an
   * empty descriptor.
   * @since 1.6
   */
  public MBeanInfo(String className,
   ...
   * @return true if and only if <code>o</code> is an MBeanInfo that is equal to this one according
   * to the rules above.
   */
  @Override
   ...
   *
   * @serialData For compatibility reasons, an object of this class is serialized as follows. <p>
   * The method {@link ObjectOutputStream#defaultWriteObject defaultWriteObject()} is called first
   * to serialize the object except the field {@code descriptor} which is declared as transient. The
   * field {@code descriptor} is serialized as follows: <ul> <li> If {@code descriptor} is an
   * instance of the class {@link ImmutableDescriptor}, the method {@link ObjectOutputStream#write
   * write(int val)} is called to write a byte with the value {@code 1}, then the method {@link
   * ObjectOutputStream#writeObject writeObject(Object obj)} is called twice to serialize the field
   * names and the field values of the {@code descriptor}, respectively as a {@code String[]} and an
   * {@code Object[]};</li> <li> Otherwise, the method {@link ObjectOutputStream#write write(int
   * val)} is called to write a byte with the value {@code 0}, then the method {@link
   * ObjectOutputStream#writeObject writeObject(Object obj)} is called to serialize the field {@code
   * descriptor} directly. </ul>
   * @since 1.6
   */
   ...
   *
   * @serialData For compatibility reasons, an object of this class is deserialized as follows. <p>
   * The method {@link ObjectInputStream#defaultReadObject defaultReadObject()} is called first to
   * deserialize the object except the field {@code descriptor}, which is not serialized in the
   * default way. Then the method {@link ObjectInputStream#read read()} is called to read a byte,
   * the field {@code descriptor} is deserialized according to the value of the byte value: <ul>
   * <li>1. The method {@link ObjectInputStream#readObject readObject()} is called twice to obtain
   * the field names (a {@code String[]}) and the field values (a {@code Object[]}) of the {@code
   * descriptor}. The two obtained values then are used to construct an {@link ImmutableDescriptor}
   * instance for the field {@code descriptor};</li> <li>0. The value for the field {@code
   * descriptor} is obtained directly by calling the method {@link ObjectInputStream#readObject
   * readObject()}. If the obtained value is null, the field {@code descriptor} is set to {@link
   * ImmutableDescriptor#EMPTY_DESCRIPTOR EMPTY_DESCRIPTOR};</li> <li>-1. This means that there is
   * no byte to read and that the object is from an earlier version of the JMX API. The field {@code
   * descriptor} is set to {@link ImmutableDescriptor#EMPTY_DESCRIPTOR EMPTY_DESCRIPTOR}.</li>
   * <li>Any other value. A {@link StreamCorruptedException} is thrown.</li> </ul>
   * @since 1.6
   */
