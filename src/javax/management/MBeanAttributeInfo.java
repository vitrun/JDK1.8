...
public class MBeanAttributeInfo extends MBeanFeatureInfo implements Cloneable {
   ...
   * @throws IllegalArgumentException if {@code isIs} is true but {@code isReadable} is not, or if
   * {@code isIs} is true and {@code type} is not {@code boolean} or {@code java.lang.Boolean}. (New
   * code should always use {@code boolean} rather than {@code java.lang.Boolean}.)
   */
   ...
   * @return true if and only if <code>o</code> is an MBeanAttributeInfo such that its {@link
   * #getName()}, {@link #getType()}, {@link #getDescription()}, {@link #isReadable()}, {@link
   * #isWritable()}, and {@link #isIs()} values are equal (not necessarily identical) to those of
   * this MBeanAttributeInfo.
   */
