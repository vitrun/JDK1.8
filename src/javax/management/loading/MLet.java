...
/**
 * Allows you to instantiate and register one or several MBeans in the MBean server coming from a
 * remote URL. M-let is a shortcut for management applet. The m-let service does this by loading an
 * m-let text file, which specifies information on the MBeans to be obtained. The information on
 * each MBean is specified in a single instance of a tag, called the MLET tag. The location of the
 * m-let text file is specified by a URL. <p> The <CODE>MLET</CODE> tag has the following syntax:
 * <p> &lt;<CODE>MLET</CODE><BR> <CODE>CODE = </CODE><VAR>class</VAR><CODE> | OBJECT =
 * </CODE><VAR>serfile</VAR><BR> <CODE>ARCHIVE = &quot;</CODE><VAR>archiveList</VAR><CODE>&quot;</CODE><BR>
 * <CODE>[CODEBASE = </CODE><VAR>codebaseURL</VAR><CODE>]</CODE><BR> <CODE>[NAME =
 * </CODE><VAR>mbeanname</VAR><CODE>]</CODE><BR> <CODE>[VERSION = </CODE><VAR>version</VAR><CODE>]</CODE><BR>
 * &gt;<BR> <CODE>[</CODE><VAR>arglist</VAR><CODE>]</CODE><BR> &lt;<CODE>/MLET</CODE>&gt; <p> where:
 * <DL> <DT><CODE>CODE = </CODE><VAR>class</VAR></DT> <DD> This attribute specifies the full Java
 * class name, including package name, of the MBean to be obtained. The compiled <CODE>.class</CODE>
 * file of the MBean must be contained in one of the <CODE>.jar</CODE> files specified by the
 * <CODE>ARCHIVE</CODE> attribute. Either <CODE>CODE</CODE> or <CODE>OBJECT</CODE> must be present.
 * </DD> <DT><CODE>OBJECT = </CODE><VAR>serfile</VAR></DT> <DD> This attribute specifies the
 * <CODE>.ser</CODE> file that contains a serialized representation of the MBean to be obtained.
 * This file must be contained in one of the <CODE>.jar</CODE> files specified by the
 * <CODE>ARCHIVE</CODE> attribute. If the <CODE>.jar</CODE> file contains a directory hierarchy,
 * specify the path of the file within this hierarchy. Otherwise  a match will not be found. Either
 * <CODE>CODE</CODE> or <CODE>OBJECT</CODE> must be present. </DD> <DT><CODE>ARCHIVE =
 * &quot;</CODE><VAR>archiveList</VAR><CODE>&quot;</CODE></DT> <DD> This mandatory attribute
 * specifies one or more <CODE>.jar</CODE> files containing MBeans or other resources used by the
 * MBean to be obtained. One of the <CODE>.jar</CODE> files must contain the file specified by the
 * <CODE>CODE</CODE> or <CODE>OBJECT</CODE> attribute. If archivelist contains more than one file:
 * <UL> <LI>Each file must be separated from the one that follows it by a comma (,).
 * <LI><VAR>archivelist</VAR> must be enclosed in double quote marks. </UL> All <CODE>.jar</CODE>
 * files in <VAR>archivelist</VAR> must be stored in the directory specified by the code base URL.
 * </DD> <DT><CODE>CODEBASE = </CODE><VAR>codebaseURL</VAR></DT> <DD> This optional attribute
 * specifies the code base URL of the MBean to be obtained. It identifies the directory that
 * contains the <CODE>.jar</CODE> files specified by the <CODE>ARCHIVE</CODE> attribute. Specify
 * this attribute only if the <CODE>.jar</CODE> files are not in the same directory as the m-let
 * text file. If this attribute is not specified, the base URL of the m-let text file is used. </DD>
 * <DT><CODE>NAME = </CODE><VAR>mbeanname</VAR></DT> <DD> This optional attribute specifies the
 * object name to be assigned to the MBean instance when the m-let service registers it. If
 * <VAR>mbeanname</VAR> starts with the colon character (:), the domain part of the object name is
 * the default domain of the MBean server, as returned by {@link javax.management.MBeanServer#getDefaultDomain()}.
 * </DD> <DT><CODE>VERSION = </CODE><VAR>version</VAR></DT> <DD> This optional attribute specifies
 * the version number of the MBean and associated <CODE>.jar</CODE> files to be obtained. This
 * version number can be used to specify that the <CODE>.jar</CODE> files are loaded from the server
 * to update those stored locally in the cache the next time the m-let text file is loaded.
 * <VAR>version</VAR> must be a series of non-negative decimal integers each separated by a period
 * from the one that precedes it. </DD> <DT><VAR>arglist</VAR></DT> <DD> This optional attribute
 * specifies a list of one or more parameters for the MBean to be instantiated. This list describes
 * the parameters to be passed the MBean's constructor. Use the following syntax to specify each
 * item in <VAR>arglist</VAR>: <DL> <DT>&lt;<CODE>ARG TYPE=</CODE><VAR>argumentType</VAR>
 * <CODE>VALUE=</CODE><VAR>value</VAR>&gt;</DT> <DD>where: <UL> <LI><VAR>argumentType</VAR> is the
 * type of the argument that will be passed as parameter to the MBean's constructor.</UL> </DD>
 * </DL> <P>The arguments' type in the argument list should be a Java primitive type or a Java basic
 * type (<CODE>java.lang.Boolean, java.lang.Byte, java.lang.Short, java.lang.Long,
 * java.lang.Integer, java.lang.Float, java.lang.Double, java.lang.String</CODE>). </DD> </DL>
 *
 * When an m-let text file is loaded, an instance of each MBean specified in the file is created and
 * registered. <P> The m-let service extends the <CODE>java.net.URLClassLoader</CODE> and can be
 * used to load remote classes and jar files in the VM of the agent. <p><STRONG>Note - </STRONG> The
 * <CODE>MLet</CODE> class loader uses the {@link javax.management.MBeanServerFactory#getClassLoaderRepository(javax.management.MBeanServer)}
...
public class MLet extends java.net.URLClassLoader
   ...
   *
   * @param urls The URLs from which to load classes and resources.
   * @param delegateToCLR True if, when a class is not found in either the parent ClassLoader or the
   * URLs, the MLet should delegate to its containing MBeanServer's {@link ClassLoaderRepository}.
   */
   ...
   * @return A set containing one entry per MLET tag in the m-let text file loaded. Each entry
   * specifies either the ObjectInstance for the created MBean, or a throwable object (that is, an
   * error or an exception) if the MBean could not be created.
   * @throws ServiceNotFoundException One of the following errors has occurred: The m-let text file
   * does not contain an MLET tag, the m-let text file is not found, a mandatory attribute of the
   ...
   * @throws ServiceNotFoundException One of the following errors has occurred: The m-let text file
   * does not contain an MLET tag, the m-let text file is not found, a mandatory attribute of the
   * MLET tag is not specified, the url is malformed.
   * @throws IllegalStateException MLet MBean is not registered with an MBeanServer.
   */
   ...
   * @throws UnsupportedOperationException if this implementation does not support storing native
   ...
   * the MBean server. The value false means that either the registration phase has failed.
   */
   ...
   * @throws UnsupportedOperationException If this implementation does not support this operation.
   */
   ...
   * @throws ClassNotFoundException if the class for the object being restored cannot be found.
   * @throws UnsupportedOperationException if this implementation does not support this operation.
   */
   ...
   * @param clr The ClassLoaderRepository that will be used to search for the given class, if it is
   * not found in this ClassLoader.  May be null.
   * @return The resulting Class object.
   * @throws ClassNotFoundException The specified class could not be found in this ClassLoader nor
   * in the given ClassLoaderRepository.
   */
   ...
   * @param clr The ClassLoaderRepository that can be used to search for the given class. This
   * parameter is <code>null</code> when called from within the {@link
   ...
   *
   * @param version The version number of the <CODE>.jar</CODE> file stored locally.
   ...
   * @param mlet The <CODE>MLetContent</CODE> instance that represents the <CODE>MLET</CODE> tag.
   * @return the codebase to use for the loaded MBean.  The returned value should not be null.
   * @throws Exception if the MBean is not to be loaded for some reason.  The exception will be
   * added to the set returned by {@link #getMBeansFromURL getMBeansFromURL}.
   */
   ...
   * @throws ClassNotFoundException The specified serialized object could not be found.
   * @throws IOException An I/O error occurred while loading serialized object.
   */
