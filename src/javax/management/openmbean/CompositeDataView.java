...
public interface CompositeDataView {
   ...
   *
   * @param ct The expected {@code CompositeType} of the returned value.  If the returned value is
   * {@code cd}, then {@code cd.getCompositeType().equals(ct)} should be true. Typically this will
   * be because {@code cd} is a {@link CompositeDataSupport} constructed with {@code ct} as its
   * {@code CompositeType}.
