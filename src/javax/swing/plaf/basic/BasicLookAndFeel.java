...
public abstract class BasicLookAndFeel extends LookAndFeel implements Serializable {
   ...
   *
   * @param table the {@code UIDefaults} object the values are added to
   * @param systemColors array of {@code name-color} pairs as described in {@link
   * #initSystemColorDefaults(UIDefaults)}
   * @param useNative whether the color is obtained from {@code SystemColor} or {@code
   * Color.decode}
   * @throws NullPointerException if {@code systemColors} is {@code null}; or {@code systemColors}
   * is not empty, and {@code table} is {@code null}; or one of the names of the {@code name-color}
   * pairs is {@code null}; or {@code useNative} is {@code false} and one of the {@code colors} of
   * the {@code name-color} pairs is {@code null}
   * @throws ArrayIndexOutOfBoundsException if {@code useNative} is {@code false} and {@code
   * systemColors.length} is odd
   ...
   *
   * @return an ActionMap containing {@code Actions} responsible for playing auditory cues
   * @throws ClassCastException if the value of the default {@code "AuditoryCues.actionMap"} is not
   * an {@code ActionMap}, or the value of the default {@code "AuditoryCues.cueList"} is not an
   * {@code Object[]}
   ...
   * @return an {@code Action} used to play the source, or {@code null} if {@code key} is {@code
   * null}
   * @see #playSound(Action)
   * @since 1.4
   */
   ...
   *
   * @param soundFile the name of the audio file to be retrieved from disk
   ...
   *
   * @param audioAction an Action that knows how to render the audio associated with the system or
   * user activity that is occurring; a value of {@code null}, is ignored
   * @throws ClassCastException if {@code audioAction} is {@code non-null} and the value of the
   * default {@code "AuditoryCues.playList"} is not an {@code Object[]}
   * @since 1.4
   */
