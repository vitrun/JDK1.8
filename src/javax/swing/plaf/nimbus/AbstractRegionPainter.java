...
public abstract class AbstractRegionPainter implements Painter<JComponent> {
  ...
  public final void paint(Graphics2D g, JComponent c, int w, int h) {
    ...
    PaintContext.CacheMode cacheMode =
        ctx == null ? PaintContext.CacheMode.NO_CACHING : ctx.cacheMode;
   ...
   * defaults.
   *
   * @param key A key corresponding to the value in the UI Defaults table of UIManager where the
   * base color is defined
   ...
   * @return The derived color, whose color value will change if the parent uiDefault color changes.
   */
   ...
   * @throws NullPointerException if {@code midpoints} array is null, or {@code colors} array is
   * null,
   * @throws IllegalArgumentException if start and end points are the same points, or {@code
   * midpoints.length != colors.length}, or {@code colors} is less than 2 in size, or a {@code
   * midpoints} value is less than 0.0 or greater than 1.0, or the {@code midpoints} are not
   ...
   * @throws NullPointerException if {@code midpoints} array is null, or {@code colors} array is
   * null
   * @throws IllegalArgumentException if {@code r} is non-positive, or {@code midpoints.length !=
   * colors.length}, or {@code colors} is less than 2 in size, or a {@code midpoints} value is less
   * than 0.0 or greater than 1.0, or the {@code midpoints} are not provided in strictly increasing
   * order
   */
  ...
  protected static class PaintContext {
     ...
     * @param canvasSize The size of the canvas used when encoding the various x/y values. May be
     * null. If null, then it is assumed that there are no encoded values, and any calls to one of
     * the "decode" methods will return the passed in value.
     ...
     * @param maxH The maximum scale in the horizontal direction to use before punting and redrawing
     * from scratch. For example, if maxH is 2, then we will attempt to scale any cached images up
     * to 2x the canvas width before redrawing from scratch. Reasonable maxH values may improve
     * painting performance. If set too high, then you may get poor looking graphics at higher zoom
     * levels. Must be &gt;= 1.
     * @param maxV The maximum scale in the vertical direction to use before punting and redrawing
     * from scratch. For example, if maxV is 2, then we will attempt to scale any cached images up
     * to 2x the canvas height before redrawing from scratch. Reasonable maxV values may improve
     * painting performance. If set too high, then you may get poor looking graphics at higher zoom
     * levels. Must be &gt;= 1.
     */
    public PaintContext(Insets insets, Dimension canvasSize, boolean inverted,
  ...
  private void prepare(float w, float h) {
    ...
    rightScale =
        (ctx.canvasSize.width - ctx.b) == 0f ? 0f : rightWidth / (ctx.canvasSize.width - ctx.b);
    topScale = ctx.c == 0f ? 0f : topHeight / ctx.c;
    centerVScale = (ctx.d - ctx.c) == 0f ? 0f : centerHeight / (ctx.d - ctx.c);
    bottomScale =
        (ctx.canvasSize.height - ctx.d) == 0f ? 0f : bottomHeight / (ctx.canvasSize.height - ctx.d);
  }
  ...
  /**
   * Gets the rendered image for this painter at the requested size, either from cache or create a
   * new one
   */
  private VolatileImage getImage(GraphicsConfiguration config, JComponent c,
    ...
    VolatileImage buffer = (VolatileImage) imageCache
        .getImage(config, w, h, this, extendedCacheKeys);
