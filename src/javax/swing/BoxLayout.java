...
public class BoxLayout implements LayoutManager2, Serializable {
   ...
   * <code>BoxLayout.Y_AXIS</code>, <code>BoxLayout.LINE_AXIS</code> or
   * <code>BoxLayout.PAGE_AXIS</code>
   * @param dbg the stream to which debugging messages should be sent, null if none
   */
  ...
  public Dimension preferredLayoutSize(Container target) {
    ...
    size.width = (int) Math
        .min((long) size.width + (long) insets.left + (long) insets.right, Integer.MAX_VALUE);
    size.height = (int) Math
        .min((long) size.height + (long) insets.top + (long) insets.bottom, Integer.MAX_VALUE);
    return size;
  }
