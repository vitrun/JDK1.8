...
public interface Soundbank {
   ...
   *
   * @return an array of resources, excluding instruments.  If the sound bank contains no resources
   * (other than instruments), returns an array of length 0.
   */
   ...
   *
   * @return an array of the <code>Instruments</code> in this <code>SoundBank</code> If the sound
   * bank contains no instruments, returns an array of length 0.
   ...
   *
   * @param patch a <code>Patch</code> object specifying the bank index and program change number
   * @return the requested instrument, or <code>null</code> if the sound bank doesn't contain that
   * instrument
   * @see #getInstruments
   * @see Synthesizer#loadInstruments(Soundbank, Patch[])
   */
