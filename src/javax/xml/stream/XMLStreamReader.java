...
public interface XMLStreamReader extends XMLStreamConstants {
   ...
   * chunk, or it may split it into several chunks. If the property javax.xml.stream.isCoalescing is
   * set to true element content must be coalesced and only one CHARACTERS event must be returned
   * for contiguous element content or CDATA Sections.
   *
   * By default entity references must be expanded and reported transparently to the application. An
   * exception will be thrown if an entity reference cannot be expanded. If element content is empty
   * (i.e. content is "") then no CHARACTERS event will be reported.
   *
   * <p>Given the following XML:<br> &lt;foo>&lt;!--description-->content
   * text&lt;![CDATA[&lt;greeting>Hello&lt;/greeting>]]>other content&lt;/foo><br> The behavior of
   * calling next() when being on foo will be:<br> 1- the comment (COMMENT)<br> 2- then the
   * characters section (CHARACTERS)<br> 3- then the CDATA section (another CHARACTERS)<br> 4- then
   * the next characters section (another CHARACTERS)<br> 5- then the END_ELEMENT<br>
   *
   ...
   * namespace and name of the current event.  If the namespaceURI is null it is not checked for
   * equality, if the localName is null it is not checked for equality.
   *
   ...
   * coalesced content. <br /> Precondition: the current event is START_ELEMENT. <br />
   * Postcondition: the current event is the corresponding END_ELEMENT.
   *
   * <br />The method does the following (implementations are free to optimized but must do
   ...
   * </pre>
   *
   * @throws XMLStreamException if the current event is not a START_ELEMENT or if a non text element
   * is encountered
   */
  public String getElementText() throws XMLStreamException;

  /**
   * Skips any white space (isWhiteSpace() returns true), COMMENT, or PROCESSING_INSTRUCTION, until
   * a START_ELEMENT or END_ELEMENT is reached. If other than white space characters, COMMENT,
   * PROCESSING_INSTRUCTION, START_ELEMENT, END_ELEMENT are encountered, an exception is thrown.
   * This method should be used when processing element-only content seperated by white space.
   *
   * <br /> Precondition: none <br /> Postcondition: the current event is START_ELEMENT or
   * END_ELEMENT and cursor may have moved over any whitespace event.
   *
   * <br />Essentially it does the following (implementations are free to optimized but must do
   * equivalent processing):
   * <pre>
   * int eventType = next();
   ...
   * "sourceStart" is copied into "target" starting at "targetStart". Up to "length" characters are
   * copied.  The number of characters actually copied is returned.
   *
   * The "sourceStart" argument must be greater or equal to 0 and less than or equal to the number
   * of characters associated with the event.  Usually, one requests text starting at a
   * "sourceStart" of 0. If the number of characters actually copied is less than the "length", then
   * there is no more text. Otherwise, subsequent calls need to be made until all text has been
   * retrieved. For example:
   *
   * <code> int length = 1024; char[] myBuffer = new char[ length ];
   *
   * for ( int sourceStart = 0 ; ; sourceStart += length ) { int nCopied = stream.getTextCharacters(
   * sourceStart, myBuffer, 0, length );
   *
   * if (nCopied < length) break; } </code> XMLStreamException may be thrown if there are any XML
   * errors in the underlying source. The "targetStart" argument must be greater than or equal to 0
   * and less than the length of "target", Length must be greater than 0 and "targetStart + length"
   * must be less than or equal to length of "target".
   *
