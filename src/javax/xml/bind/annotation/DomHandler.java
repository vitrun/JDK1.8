...
public interface DomHandler<ElementT, ResultT extends Result> {
   ...
   *
   * @param errorHandler if any error happens between the invocation of this method and the
   * invocation of {@link #getElement(Result)}, they must be reported to this handler.
   *
   ...
   * @return null if the operation fails. The error must have been reported to the error handler.
   */
   ...
   *
   * @param errorHandler Receives any errors happened during the process of converting an element
   * into a {@link Source}.
   *
   * The caller must provide a non-null error handler.
   * @return null if there was an error. The error should have been reported to the handler.
   */
