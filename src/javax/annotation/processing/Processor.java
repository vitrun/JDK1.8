...
public interface Processor {
   ...
   * Since completions are being asked for, the information provided about the annotation may be
   * incomplete, as if for a source code fragment. A processor may return an empty iterable.
   * Annotation processors should focus their efforts on providing completions for annotation
   * members with additional validity constraints known to the processor, for example an {@code int}
   * member whose value should lie between 1 and 10 or a string member that should be recognized by
   * a known grammar, such as a regular expression or a URL.
   *
   * <p>Since incomplete programs are being modeled, some of the parameters may only have partial
   * information or may be {@code null}.  At least one of {@code element} and {@code userText} must
   * be non-{@code null}.  If {@code element} is non-{@code null}, {@code annotation} and {@code
   * member} may be {@code null}.  Processors may not throw a {@code NullPointerException} if some
   * parameters are {@code null}; if a processor has no completions to offer based on the provided
   * information, an empty iterable can be returned.  The processor may also return a single
   * completion with an empty value string and a message describing why there are no completions.
   *
   * <p>Completions are informative and may reflect additional validity checks performed by
   * annotation processors.  For example, consider the simple annotation:
   *
   * <blockquote>
   * <pre>
   * &#064;MersennePrime {
   ...
   * </blockquote>
   *
   * (A Mersenne prime is prime number of the form 2<sup><i>n</i></sup> - 1.) Given an {@code
   * AnnotationMirror} for this annotation type, a list of all such primes in the {@code int} range
   * could be returned without examining any other arguments to {@code getCompletions}:
   *
   * <blockquote>
   * <pre>
   ...
   * <blockquote>
   * <pre>
   * return Arrays.asList({@link Completions#of(String, String) of}(&quot;3&quot;,
   * &quot;M2&quot;),
   *                      of(&quot;7&quot;,          &quot;M3&quot;),
   *                      of(&quot;31&quot;,         &quot;M5&quot;),
   *                      of(&quot;127&quot;,        &quot;M7&quot;),
   ...
   * </blockquote>
   *
   * However, if the {@code userText} is available, it can be checked to see if only a subset of the
   * Mersenne primes are valid.  For example, if the user has typed
   *
   * <blockquote> <code> &#064;MersennePrime(1 </code> </blockquote>
   *
   * the value of {@code userText} will be {@code "1"}; and only two of the primes are possible
   * completions:
   *
   * <blockquote>
   * <pre>
   ...
   * </blockquote>
   *
   * Sometimes no valid completion is possible.  For example, there is no in-range Mersenne prime
   * starting with 9:
   *
   * <blockquote> <code> &#064;MersennePrime(9 </code> </blockquote>
   *
