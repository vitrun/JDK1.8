...
public interface StatementEventListener extends java.util.EventListener {
  ...
  /**
   * The driver calls this method on all <code>StatementEventListener</code>s registered on the
   * connection when it detects that a <code>PreparedStatement</code> is closed.
   *
   ...
   * <p>
   *
   * @param event an event object describing the source of the event, the statement that is invalid
   * and the exception the driver is about to throw.  The source of the event is the
   * <code>PooledConnection</code> which the invalid <code>PreparedStatement</code> is associated
   * with. <p>
   * @since 1.6
   */
