...
public class BatchUpdateException extends SQLException {
   ...
   * @param updateCounts an array of <code>int</code>, with each element indicating the update
   * count, <code>Statement.SUCCESS_NO_INFO</code> or <code>Statement.EXECUTE_FAILED</code> for each
   * SQL command in the batch for JDBC drivers that continue processing after a command failure; an
   * update count or <code>Statement.SUCCESS_NO_INFO</code> for each SQL command in the batch prior
   * to the failure for JDBC drivers that stop processing after a command failure
   ...
   * SQL command in the batch for JDBC drivers that continue processing after a command failure; an
   * update count or <code>Statement.SUCCESS_NO_INFO</code> for each SQL command in the batch prior
   * to the failure for JDBC drivers that stop processing after a command failure <p>
   * <strong>Note:</strong> There is no validation of {@code updateCounts} for overflow and because
   * of this it is recommended that you use the constructor {@code BatchUpdateException(String
   * reason, String SQLState, int vendorCode, long []updateCounts,Throwable cause) }. </p>
   * @param cause the underlying reason for this <code>SQLException</code> (which is saved for later
   * retrieval by the <code>getCause()</code> method); may be null indicating the cause is
   * non-existent or unknown.
   * @see #BatchUpdateException(java.lang.String, java.lang.String, int, long[],
   ...
   * commands after an error, one of the following for every command in the batch: <OL> <LI>an
   * update count <LI><code>Statement.SUCCESS_NO_INFO</code> to indicate that the command executed
   * successfully but the number of rows affected is unknown <LI><code>Statement.EXECUTE_FAILED</code>
   * to indicate that the command failed to execute successfully </OL>
