...
public final class Math {
  ...
  /**
   * Returns the floor modulus of the {@code int} arguments. <p> The floor modulus is {@code x -
   * (floorDiv(x, y) * y)}, has the same sign as the divisor {@code y}, and is in the range of
   * {@code -abs(y) < r < +abs(y)}.
   *
   * <p> The relationship between {@code floorDiv} and {@code floorMod} is such that: <ul>
   * <li>{@code floorDiv(x, y) * y + floorMod(x, y) == x} </ul> <p> The difference in values between
   * {@code floorMod} and the {@code %} operator is due to the difference between {@code floorDiv}
   * that returns the integer less than or equal to the quotient and the {@code /} operator that
   * returns the integer closest to zero. <p> Examples: <ul> <li>If the signs of the arguments are
   * the same, the results of {@code floorMod} and the {@code %} operator are the same.  <br> <ul>
   * <li>{@code floorMod(4, 3) == 1}; &nbsp; and {@code (4 % 3) == 1}</li> </ul> <li>If the signs of
   * the arguments are different, the results differ from the {@code %} operator.<br> <ul>
   * <li>{@code floorMod(+4, -3) == -2}; &nbsp; and {@code (+4 % -3) == +1} </li> <li>{@code
   * floorMod(-4, +3) == +2}; &nbsp; and {@code (-4 % +3) == -1} </li> <li>{@code floorMod(-4, -3)
   * == -1}; &nbsp; and {@code (-4 % -3) == -1 } </li> </ul> </li> </ul> <p> If the signs of
   * arguments are unknown and a positive modulus is needed it can be computed as {@code
   ...
   * </ul>
   *
   * <p>The computed result must be within 2.5 ulps of the exact result. The result of {@code tanh}
   * for any finite input must have an absolute value less than or equal to 1.  Note that once the
   * exact result of tanh is within 1/2 of an ulp of the limit value of &plusmn;1, correctly signed
