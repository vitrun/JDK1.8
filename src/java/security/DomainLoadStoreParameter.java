...
public final class DomainLoadStoreParameter implements LoadStoreParameter {
   ...
   *
   * @param configuration identifier for the domain configuration data. The name of the target
   * domain should be specified in the {@code java.net.URI} fragment component when it is necessary
   * to distinguish between several domain configurations at the same location.
   * @param protectionParams the map from keystore name to the parameter used to protect keystore
   * data. A {@code java.util.Collections.EMPTY_MAP} should be used when protection parameters are
   * not required or when they have been specified by properties in the domain configuration data.
   * It is cloned to prevent subsequent modification.
   * @throws NullPointerException if {@code configuration} or {@code protectionParams} is {@code
   * null}
   */
