...
public class PolicyQualifierInfo {
   ...
   * @throws IOException thrown if the byte array does not represent a valid and parsable policy
   * qualifier
   */
   ...
   *
   * @return the ASN.1 DER encoded bytes (never {@code null}). Note that a copy is returned, so the
   * data is cloned each time this method is called.
   */
   ...
   *
   * @return the ASN.1 DER encoded bytes of the {@code qualifier} field. Note that a copy is
   * returned, so the data is cloned each time this method is called.
   */
   ...
   * {@code PolicyQualifierInfo}.
   *
   * @return a {@code String} describing the contents of this {@code PolicyQualifierInfo}
   */
