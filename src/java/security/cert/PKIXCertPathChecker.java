...
public abstract class PKIXCertPathChecker
   ...
   *
   * @param forward the order that certificates are presented to the {@code check} method. If {@code
   * true}, certificates are presented from target to most-trusted CA (forward); if {@code false},
   * from most-trusted CA to target (reverse).
   * @throws CertPathValidatorException if this {@code PKIXCertPathChecker} is unable to check
   * certificates in the specified order; it should never be thrown if the forward flag is false
   * since reverse checking must be supported
   */
  @Override
   ...
   *
   * @return {@code true} if forward checking is supported, {@code false} otherwise
   */
  @Override
   ...
   *
   * @return an immutable {@code Set} of X.509 extension OIDs (in {@code String} format) supported
   * by this {@code PKIXCertPathChecker}, or {@code null} if no extensions are supported
   */
   ...
   * @param unresolvedCritExts a {@code Collection} of OID strings representing the current set of
   * unresolved critical extensions
   * @throws CertPathValidatorException if the specified certificate does not pass the check
   */
