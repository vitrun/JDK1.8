...
public class KeyEvent extends InputEvent {
   ...
   * class description for {@link KeyEvent}
   * @param when A long integer that specifies the time the event occurred. Passing negative or zero
   * value is not recommended
   * @param modifiers The modifier keys down during event (shift, ctrl, alt, meta). Passing negative
   * value is not recommended. Zero value means that no modifiers were passed. Use either an
   * extended _DOWN_MASK or old _MASK modifiers, however do not mix models in the one event. The
   * extended modifiers are preferred for using
   ...
   * key-pressed and key-released events which do not map to a valid Unicode character)
   * @param keyLocation Identifies the key location.  The only legal values are
   * <code>KEY_LOCATION_UNKNOWN</code>, <code>KEY_LOCATION_STANDARD</code>,
   * <code>KEY_LOCATION_LEFT</code>, <code>KEY_LOCATION_RIGHT</code>, and
   * <code>KEY_LOCATION_NUMPAD</code>.
   * @throws IllegalArgumentException if <code>id</code> is <code>KEY_TYPED</code> and
   * <code>keyChar</code> is <code>CHAR_UNDEFINED</code>; or if <code>id</code> is
   * <code>KEY_TYPED</code> and <code>keyCode</code> is not <code>VK_UNDEFINED</code>; or if
   * <code>id</code> is <code>KEY_TYPED</code> and <code>keyLocation</code> is not
   * <code>KEY_LOCATION_UNKNOWN</code>; or if <code>keyLocation</code> is not one of the legal
   ...
   * @throws IllegalArgumentException if <code>id</code> is <code>KEY_TYPED</code> and
   * <code>keyChar</code> is <code>CHAR_UNDEFINED</code>; or if <code>id</code> is
   * <code>KEY_TYPED</code> and <code>keyCode</code> is not <code>VK_UNDEFINED</code>
   * @throws IllegalArgumentException if <code>source</code> is null
   * @see #getSource()
   * @see #getID()
   * @see #getWhen()
   ...
   *
   * @return the integer code for an actual key on the keyboard. (For <code>KEY_TYPED</code> events,
   * the keyCode is <code>VK_UNDEFINED</code>.)
   */
   ...
   *
   * @return the Unicode character defined for this key event. If no valid Unicode character exists
   * for this key event, <code>CHAR_UNDEFINED</code> is returned.
   */
   ...
   *
   * @param keyChar a char corresponding to to the combination of keystrokes that make up this
   * event.
   */
   ...
   *
   * @return the location of the key that was pressed or released. Always returns
   * <code>KEY_LOCATION_UNKNOWN</code> for <code>KEY_TYPED</code> events.
   * @since 1.4
   */
   ...
   *
   * @return string a text description of the combination of modifier keys that were held down
   * during the event
   ...
   * not a modifier key.
   *
   * @return <code>true</code> if the key is an "action" key, <code>false</code> otherwise
   */
   ...
   *
   * @return for a unicode character with a corresponding {@code VK_} constant -- this {@code VK_}
   * constant; for a character appearing on the primary level of a known keyboard layout -- a unique
   * integer. If a character does not appear on the primary level of a known keyboard, {@code
