...
public final class LineBreakMeasurer {
   ...
   *
   * @param text the text for which this <code>LineBreakMeasurer</code> produces
   * <code>TextLayout</code> objects; the text must contain at least one character; if the text
   ...
   * <code>insertChar</code> or <code>deleteChar</code> are invoked afterward - see below)
   ...
   * <code>LineBreakMeasurer</code> instance are undefined (except, in some cases, when
   * <code>insertChar</code> or <code>deleteChar</code> are invoked afterward - see below)
   * @param breakIter the {@link BreakIterator} which defines line breaks
   * @param frc contains information about a graphics device which is needed to measure the text
   * correctly; text measurements can vary slightly depending on the device resolution, and
   * attributes such as antialiasing; this parameter does not specify a translation between the
   * <code>LineBreakMeasurer</code> and user space
   ...
   *
   * @param wrappingWidth the maximum visible advance permitted for the text in the next layout
   * @return an offset in the text representing the limit of the next <code>TextLayout</code>.
   */
   ...
   *
   * @param wrappingWidth the maximum visible advance permitted for the text in the next layout
   * @param offsetLimit the first character that can not be included in the next layout, even if the
   * text after the limit would fit within the wrapping width; <code>offsetLimit</code> must be
   * greater than the current position
   * @param requireNextWord if <code>true</code>, the current position that is returned if the
   * entire next word does not fit within <code>wrappingWidth</code>; if <code>false</code>, the
   * offset returned is at least one greater than the current position
   * @return an offset in the text representing the limit of the next <code>TextLayout</code>
   */
   ...
   *
   * @param wrappingWidth the maximum visible advance permitted for the text in the next layout
   * @return a <code>TextLayout</code>, beginning at the current position, which represents the next
   * line fitting within <code>wrappingWidth</code>
   */
   ...
   * text after the limit would fit within the wrapping width; <code>offsetLimit</code> must be
   * greater than the current position
   * @param requireNextWord if <code>true</code>, and if the entire word at the current position
   * does not fit within the wrapping width, <code>null</code> is returned. If <code>false</code>, a
   * valid layout is returned that includes at least the character at the current position
   * @return a <code>TextLayout</code>, beginning at the current position, that represents the next
   * line fitting within <code>wrappingWidth</code>.  If the current position is at the end of the
   * text used by this <code>LineBreakMeasurer</code>, <code>null</code> is returned
   */
   ...
   *
   * @param newPosition the current position of this <code>LineBreakMeasurer</code>; the position
   * should be within the text used to construct this <code>LineBreakMeasurer</code> (or in the text
   * most recently passed to <code>insertChar</code> or <code>deleteChar</code>
   * @see #getPosition
   */
   ...
   * @param insertPos the position in the text at which the character is inserted
   ...
   * @param deletePos the position in the text at which the character is deleted
