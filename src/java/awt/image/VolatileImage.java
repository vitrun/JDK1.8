...
public abstract class VolatileImage extends Image implements Transparency {
   ...
   *
   * @return a {@link BufferedImage} representation of this <code>VolatileImage</code>
   * @see BufferedImage
   */
   ...
   *
   * @return an {@link ImageProducer} that can be used to produce the pixels for a
   ...
   *
   * @param gc a <code>GraphicsConfiguration</code> object for this image to be validated against.
   * A null gc implies that the validate method should skip the compatibility test.
   * @return <code>IMAGE_OK</code> if the image did not need validation<BR>
   * <code>IMAGE_RESTORED</code> if the image needed restoration. Restoration implies that the
   * contents of the image may have been affected and the image may need to be re-rendered.<BR>
   * <code>IMAGE_INCOMPATIBLE</code> if the image is incompatible with the
   * <code>GraphicsConfiguration</code> object passed into the <code>validate</code> method.
   * Incompatibility implies that the image may need to be recreated with a new
   * <code>Component</code> or <code>GraphicsConfiguration</code> in order to get an image that can
   * be used successfully with this <code>GraphicsConfiguration</code>. An incompatible image is not
   * checked for whether restoration was necessary, so the state of the image is unchanged after a
   * return value of <code>IMAGE_INCOMPATIBLE</code> and this return value implies nothing about
   * whether the image needs to be restored.
   ...
   *
   * @return <code>true</code> if the drawing surface needs to be restored; <code>false</code>
   * otherwise.
   */
   ...
   *
   * @return an <code>ImageCapabilities</code> object that contains the capabilities of this
   * <code>VolatileImage</code>.
   * @since 1.4
   */
