...
enum StreamOpFlag {
   ...
   * @param prevCombOpFlags previously combined stream and operation flags. The value {#link
   * INITIAL_OPS_VALUE} must be used as the seed value.
   ...
   * @return the stream flags.
   * @implSpec If the spliterator is naturally {@code SORTED} (the associated {@code Comparator} is
   * {@code null}) then the characteristic is converted to the {@link #SORTED} flag, otherwise the
