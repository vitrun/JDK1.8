...
public abstract class FileLock implements AutoCloseable {
   ...
   * @param position The position within the file at which the locked region starts; must be
   * non-negative
   * @param size The size of the locked region; must be non-negative, and the sum
   * <tt>position</tt>&nbsp;+&nbsp;<tt>size</tt> must be non-negative
   * @param shared <tt>true</tt> if this lock is shared, <tt>false</tt> if it is exclusive
   ...
   * method.
   *
   * @return The file channel, or {@code null} if the file lock was not acquired by a file channel.
   */
   ...
   *
   * @return <tt>true</tt> if lock is shared, <tt>false</tt> if it is exclusive
   */
   ...
   * @return <tt>true</tt> if, and only if, this lock and the given lock range overlap by at least
   * one byte
   */
