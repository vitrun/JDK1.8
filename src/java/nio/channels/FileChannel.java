...
public abstract class FileChannel
   ...
   * @throws UnsupportedOperationException If the {@code path} is associated with a provider that
   * does not support creating file channels, or an unsupported open option is specified, or the
   * array contains an attribute that cannot be set atomically when creating the file
   * @throws IOException If an I/O error occurs
   * @throws SecurityException If a security manager is installed and it denies an unspecified
   * permission required by the implementation. In the case of the default provider, the {@link
   * SecurityManager#checkRead(String)} method is invoked to check read access if the file is opened
   * for reading. The {@link SecurityManager#checkWrite(String)} method is invoked to check write
   * access if the file is opened for writing
   * @since 1.7
   */
   ...
   * @throws IllegalArgumentException If the set contains an invalid combination of options
   * @throws UnsupportedOperationException If the {@code path} is associated with a provider that
   * does not support creating file channels, or an unsupported open option is specified
   * @throws IOException If an I/O error occurs
   * @throws SecurityException If a security manager is installed and it denies an unspecified
   * permission required by the implementation. In the case of the default provider, the {@link
   * SecurityManager#checkRead(String)} method is invoked to check read access if the file is opened
   ...
   *
   * @return This channel's file position, a non-negative integer counting the number of bytes from
   ...
   * file's content and metadata to be written to storage; otherwise, it need only force content
   ...
   * @throws AsynchronousCloseException If another thread closes either channel while the transfer
   * is in progress
   ...
   *
   * @param mode One of the constants {@link MapMode#READ_ONLY READ_ONLY}, {@link MapMode#READ_WRITE
   * READ_WRITE}, or {@link MapMode#PRIVATE PRIVATE} defined in the {@link MapMode} class, according
   * to whether the file is to be mapped read-only, read/write, or privately (copy-on-write),
   * respectively
   * @param position The position within the file at which the mapped region is to start; must be
   * non-negative
   * @param size The size of the region to be mapped; must be non-negative and no greater than
   * {@link java.lang.Integer#MAX_VALUE}
   * @return The mapped byte buffer
   * @throws NonReadableChannelException If the <tt>mode</tt> is {@link MapMode#READ_ONLY READ_ONLY}
   * but this channel was not opened for reading
   * @throws NonWritableChannelException If the <tt>mode</tt> is {@link MapMode#READ_WRITE
   * READ_WRITE} or {@link MapMode#PRIVATE PRIVATE} but this channel was not opened for both reading
   * and writing
   * @throws IllegalArgumentException If the preconditions on the parameters do not hold
   * @throws IOException If some other I/O error occurs
   ...
   * @param shared <tt>true</tt> to request a shared lock, in which case this channel must be open
   * for reading (and possibly writing); <tt>false</tt> to request an exclusive lock, in which case
   ...
   * held by this Java virtual machine, or if another thread is already blocked in this method and
   ...
   * opened for reading
   * @throws NonWritableChannelException If <tt>shared</tt> is <tt>false</tt> but this channel was
   ...
   * @throws OverlappingFileLockException If a lock that overlaps the requested region is already
   * held by this Java virtual machine, or if another thread is already blocked in this method and
   * is attempting to lock an overlapping region of the same file
   * @throws NonWritableChannelException If this channel was not opened for writing
   * @throws IOException If some other I/O error occurs
   ...
   * @param size The size of the locked region; must be non-negative, and the sum
   * <tt>position</tt>&nbsp;+&nbsp;<tt>size</tt> must be non-negative
   * @param shared <tt>true</tt> to request a shared lock, <tt>false</tt> to request an exclusive
   * lock
   * @return A lock object representing the newly-acquired lock, or <tt>null</tt> if the lock could
