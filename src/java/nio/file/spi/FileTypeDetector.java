...
public abstract class FileTypeDetector {
   ...
   * RuntimePermission}<tt>("fileTypeDetector")</tt>
   */
   ...
   * @throws SecurityException If the implementation requires to access the file, and a security
   * manager is installed, and it denies an unspecified permission required by a file system
   * provider implementation. If the file reference is associated with the default file system
   * provider then the {@link SecurityManager#checkRead(String)} method is invoked to check read
   * access to the file.
