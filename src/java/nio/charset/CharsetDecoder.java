...
public abstract class CharsetDecoder {
   ...
   * @param averageCharsPerByte A positive float value indicating the expected number of characters
   * that will be produced for each input byte
   * @param maxCharsPerByte A positive float value indicating the maximum number of characters that
   * will be produced for each input byte
   * @param replacement The initial replacement; must not be <tt>null</tt>, must have non-zero
   * length, must not be longer than maxCharsPerByte, and must be {@linkplain #isLegalReplacement
   * legal}
   ...
   *
   * @return This decoder's current replacement, which is never <tt>null</tt> and is never empty
   */
   ...
   *
   *
   * The new replacement; must not be <tt>null</tt> and must have non-zero length
   * @return This decoder
   ...
   * of the output buffer required for a given input sequence.
   *
   * @return The maximum number of characters that will be produced per byte of input
   */
   ...
   * bytes beyond those in the given buffer
   ...
   * incomplete decoding operation
   * @throws CoderMalfunctionError If an invocation of the decodeLoop method threw an unexpected
   * exception
   */
   ...
   * #decode(ByteBuffer, CharBuffer, boolean) decode} method with a value of <tt>true</tt> for the
   ...
   *
   * @param in The input byte buffer
   * @return A newly-allocated character buffer containing the result of the decoding operation.
   * The buffer's position will be zero and its limit will follow the last character written.
   ...
   * position is not legal for this charset and the current malformed-input action is {@link
   * CodingErrorAction#REPORT}
   * @throws UnmappableCharacterException If the byte sequence starting at the input buffer's
   * current position cannot be mapped to an equivalent character sequence and the current
   ...
   * return <tt>true</tt>.  </p>
   *
   * @return <tt>true</tt> if, and only if, this decoder implements an auto-detecting charset
   */
   ...
   *
   * @return <tt>true</tt> if, and only if, this decoder has detected a specific charset
   ...
   *
   * @return The charset detected by this auto-detecting decoder, or <tt>null</tt> if the charset
