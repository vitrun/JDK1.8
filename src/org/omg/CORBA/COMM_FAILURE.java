...
/**
 * This exception is raised if communication is lost while an operation is in progress, after the
 * request was sent by the client, but before the reply from the server has been returned to the
 * client.<P> It contains a minor code, which gives more detailed information about what caused the
 * exception, and a completion status. It may also contain a string describing the exception. <P>
...
public final class COMM_FAILURE extends SystemException {
   ...
   * @param completed the completion status, which must be one of <code>COMPLETED_YES</code>,
   * <code>COMPLETED_NO</code>, or <code>COMPLETED_MAYBE</code>.
   */
